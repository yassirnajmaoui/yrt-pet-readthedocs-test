...
size_t znzread(void* buf, size_t size, size_t nmemb, znzFile file)
{
	...
	if (file->zfptr != NULL)
	{
		...
		while (remain > 0)
		{
			n2read = (remain < ZNZ_MAX_BLOCK_SIZE) ? (unsigned)remain :
			                                         ZNZ_MAX_BLOCK_SIZE;
...
size_t znzwrite(const void* buf, size_t size, size_t nmemb, znzFile file)
{
	...
	if (file->zfptr != NULL)
	{
		while (remain > 0)
		{
			n2write = (remain < ZNZ_MAX_BLOCK_SIZE) ? (unsigned)remain :
			                                          ZNZ_MAX_BLOCK_SIZE;
...
#ifdef HAVE_ZLIB
	if (file->zfptr != NULL)
		return (znz_off_t)gzseek(file->zfptr, offset, whence);
#endif
...
#ifdef HAVE_ZLIB
	if (file->zfptr != NULL)
		return (znz_off_t)gztell(file->zfptr);
#endif
